					    ...
					    obs_sceneitem_crop &crop)
{
	...
	if (d->HasKey("position")) {
		CefRefPtr<CefValue> val = d->GetValue("position");
		info.pos = DeserializeVec2(val);
	}
	if (d->HasKey("scale")) {
		CefRefPtr<CefValue> val = d->GetValue("scale");
		info.scale = DeserializeVec2(val);
	} else
		info.scale = {1, 1};
	...
	if (d->HasKey("alignment")) {
		std::string val = d->GetString("alignment");
		info.alignment = GetInt32FromAlignmentId(val);
	} else {
	...
	if (d->HasKey("boundsType")) {
		...
		if (info.bounds_type != OBS_BOUNDS_NONE) {
			if (d->HasKey("bounds")) {
				CefRefPtr<CefValue> val = d->GetValue("bounds");
				info.bounds = DeserializeVec2(val);
			}
			if (d->HasKey("boundsAlignment") &&
			    d->GetType("boundsAlignment") == VTYPE_STRING) {
				std::string val = d->GetString("boundsAlignment");
				info.bounds_alignment = GetInt32FromAlignmentId(val);
			} else
	...
	: m_parent(parent)
{
	...
	CefRefPtr<CefValue> val2 = CefValue::Create();
...
StreamElementsObsSceneManager::ObsGetUniqueSceneName(std::string name)
{
	...
	auto hasSceneName = [&](std::string name) -> bool {
		for (size_t idx = 0; idx < scenes.sources.num; ++idx) {
			...
			if (qstricmp(name.c_str(), obs_source_get_name(scene)) ==
			    0)
				return true;
		}
...
StreamElementsObsSceneManager::ObsGetUniqueSceneCollectionName(std::string name)
{
	...
	while (!isUnique) {
		...
		for (size_t idx = 0; names[idx] && isUnique; ++idx) {
			if (qstricmp(result.c_str(), names[idx]) == 0)
				isUnique = false;
		}
	...
	CefRefPtr<CefValue> input, CefRefPtr<CefValue> &output)
{
	...
	for (auto item : items) {
		if (qstricmp(item.second.c_str(), id.c_str()) == 0) {
			actualId = item.second;

			break;
		} else if (qstricmp(item.first.c_str(), id.c_str()) == 0) {
			actualId = item.second;
