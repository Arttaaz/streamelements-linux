...
void StreamElementsBrowserWidget::InitBrowserAsyncInternal()
{
	...
	CefUIThreadExecute(
		[this]() {
			...
			clientRect.x = 0;
			clientRect.y = 0;
			clientRect.width = self->width();
			clientRect.height = self->height();

			// CEF window attributes
			CefWindowInfo windowInfo;
			windowInfo.width = clientRect.width - clientRect.x;
			windowInfo.height = clientRect.height - clientRect.y;
