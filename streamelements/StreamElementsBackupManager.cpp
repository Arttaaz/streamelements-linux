	...
	if (-1 != fd) {
		...
		if (0 == zip_entry_open(zip, zipPath.c_str())) {
			...
			while (bytes_read > 0 && success) {
				int retVal = zip_entry_write(zip, buf, bytes_read);
				if (retVal < 0) {
					success = false;
...
static bool AddBufferToZip(zip_t *zip, const char *buf, size_t bufLen,
			   std::string zipPath)
{
	bool result = false;
			       ...
			       std::string timestamp)
{
	...
	return AddBufferToZip(zip, json.c_str(), json.size(), relPath);
}
			...
			if (IsQualifiedFileForRestore(name, requestProfiles,
				...
				if (MKDIR_ERROR ==
					...
					context.handle = _wopen(
						...
						_S_IREAD | _S_IWRITE);
#else
					context.handle = open(destFilePath.c_str(), O_WRONLY | O_CREAT | S_IRUSR | S_IWUSR);
#endif
					if (-1 == context.handle) {
						success = false;
					} else {
...
#else
		const char* obs_pid_buffer = std::to_string(getpid()).c_str();
#endif
		std::string cwd;
		cwd.resize(MAX_PATH);
#ifdef _WIN32
...
#else
			char **wArgv = CommandLineToArgv(GetCommandLine, &argc);
#endif
...
#else
		vars["OBS_EXE_PATH"] = obs_path_utf16;
#endif
